...
# set up workspace
rm(list=ls(all=TRUE))
setwd("~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/3 Data/subjData_prolific_pSARL_v3/data")
datadir = "~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/3 Data/subjData_prolific_pSARL_v3/data"
datadir_two = "~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/3 Data/subjData_prolific_pSARL_v2/data"
...
# read in data
flag = 2 #1 = compile all the data, 2 = read in the already ready files

if(flag == 1){
  file_paths = fs::dir_ls(datadir)
  compiledData_three = data.frame()
  for (i in seq_along(file_paths)){
    newSubjData = read_csv(file = file_paths[[i]])
    compiledData_three = rbind(compiledData_three, newSubjData)
  }

  file_paths = fs::dir_ls(datadir_two)
  compiledData_two = data.frame()
  for (i in seq_along(file_paths)){
    newSubjData = read_csv(file = file_paths[[i]])
    compiledData_two = rbind(compiledData_two, newSubjData)
  }
  
  compiledData = data.frame()
  compiledData = rbind(compiledData_two,compiledData_three)
  # clean up data
...
counter = 1
 ...
 #write.csv(testData, 'pSARL_v0203_testData_10.3.23.csv')
#  write.csv(pracData, 'SARL_pracData.csv')
  }else{
    testData = read_csv("~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/4 Analyses/OrganizedCode/Expt3/pSARL_v0203_testData_10.3.23.csv")
  }
...
rlData_SARL = read_csv("~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/3 Data/subjData_prolific_replication_v2_5.25/SARL_prolificRep_rlData_noNan_081123.csv")
testData_SARL = read_csv("~/Library/CloudStorage/Box-Box/ACTlab_projects/JT_SARL/SARL/3 Data/subjData_prolific_replication_v2_5.25/SARL_prolificRep_testData_081123.csv")
...
\newpage
# exclude blocks where they zone out
# Detrend CV and accu
```{r, echo=F, message=F, warning=F, error=T, fig.width=6, fig.height=4, fig.align='center'}
...
rlData_noNan = rlData %>% filter(is.na(longRTs))
...
```
# Correlations between attention metrics and attention x RL
## Correlation between attention metrics
```{r, echo=F, message=F, warning=F, error=T, fig.width=6, fig.height=4, fig.align='center'}
# Correlation between attention metrics
attentionmetrics = rlData %>%filter(!is.na(accu_infreq), !is.na(CV))%>% group_by(subject) %>% dplyr::summarise(correlSA = cor(accu_infreq,CV, method = 'spearman'),correlSA_detrend = cor(accuracy_infreq_detrend,coefVar_detrend, method = 'spearman'))
...
```
# SARL: detrend RTs then look at effect within block
```{r, echo=F, message=F, warning=F, error=T, fig.width=5, fig.height=15, fig.align='center'}
...
```
# SARL: Add columns excluding the first trial of each block
```{r, echo=F, message=F, warning=F, error=T, fig.width=5, fig.height=5, fig.align='center'}
...
rlData_SARL$avgRT_detrend_noone = NA
rlData_SARL$coefVar_detrend_noone = NA
rlData_SARL$accuracy_infreq_detrend_noone = NA
rlData_SARL$coefVar_prev_detrend_noone = NA
rlData_SARL$accuracy_infreq_prev_detrend_noone = NA
for(si in 1:N_SARL){
  subData = rlData_SARL %>% filter(subject == subjNums_SARL[si], !is.na(coefVar),!is.na(accuracy_infreq)) # get subject data
  sub_avgRT = c(t(subData$meanRT_freq_noone)) # get rt column and transpose
  sub_coefVar = c(t(subData$coefVar_noone)) # get cv column and transpose
  sub_accuracy_infreq = c(t(subData$accuracy_infreq_noone))# get accuracy column and transpose
  sub_coefVar_prev = c(t(subData$coefVar_prev_noone)) # get cv column and transpose
  sub_accuracy_infreq_prev = c(t(subData$accuracy_infreq_prev_noone))# get accuracy column and transpose
  ...
  rlData_SARL$avgRT_detrend_noone[rlData_SARL$subject == subjNums_SARL[si]] = sub_avgRT_detrend
  rlData_SARL$coefVar_detrend_noone[rlData_SARL$subject == subjNums_SARL[si]] = sub_coefVar_detrend
  rlData_SARL$accuracy_infreq_detrend_noone[rlData_SARL$subject == subjNums_SARL[si]] = sub_accuracy_infreq_detrend
  rlData_SARL$coefVar_prev_detrend_noone[rlData_SARL$subject == subjNums_SARL[si]] = sub_coefVar_prev_detrend
  rlData_SARL$accuracy_infreq_prev_detrend_noone[rlData_SARL$subject == subjNums_SARL[si]] = sub_accuracy_infreq_prev_detrend  
}
...
```{r, echo=F, message=F, warning=F, error=T, fig.width=6, fig.height=4, fig.align='center'}
#accuReward = rlData_noNan %>% group_by(subject,rewardSign) %>% dplyr::summarize(avgAcc = mean(accuracy_infreq,na.rm = TRUE)) #*** update when you do rl code
cvrewardsign_detrend = rlData_SARL %>% group_by(subject,rewardSign) %>% dplyr::summarize(cv_detrend = mean(coefVar_detrend,na.rm = TRUE),cv_detrend_noone = mean(coefVar_detrend_noone,na.rm = TRUE))
summaryStats_cvrewardsign_detrend_noone = Rmisc::summarySEwithin(cvrewardsign_detrend,measurevar="cv_detrend_noone", withinvars=c("rewardSign"),idvar="subject", na.rm=FALSE, conf.interval=.95)
...
ggplot(summaryStats_cvrewardsign_detrend_noone, aes(x = rewardSign, y = cv_detrend_noone))+
  geom_point(size = 3)+
  scale_x_discrete(name = 'reward sign')+
  labs(y = 'cv_detrend_noone',x = 'reward sign')+
  theme(axis.title.x = element_text(size = 22, face = 'bold'), 
        axis.title.y = element_text(size = 22, face = 'bold'),
        axis.text.x = element_text(size = 20),
        axis.text.y = element_text(size = 20), 
        ...
        panel.background = element_rect(fill = "white", colour = "black"),
        panel.border = element_rect(fill = NA, colour = "white"))+
  geom_errorbar(aes(ymin=cv_detrend_noone-se, ymax=cv_detrend_noone+se),size = 1.5, width=0)
#ggsave('rewardsigncv_detrend.eps')

t.test(cvrewardsign_detrend$cv_detrend_noone[cvrewardsign_detrend$rewardSign == 'pos'],cvrewardsign_detrend$cv_detrend_noone[cvrewardsign_detrend$rewardSign == 'neg'],paired = TRUE)
...
rlData$meanRT_freq_noone = NA
rlData$sdRT_freq_noone = NA
rlData$accuracy_infreq_noone = NA
rlData$threeFreq_noone = NA
rlData$accuracy_infreq_prev_noone = NA
rlData$coefVar_noone = NA
rlData$coefVar_prev_noone = NA

for(ai in 1:N){
  for(bi in 2:nanMax(testData$rlNum)){
    idxRL = which(testData$subject == subjNums[ai]& testData$blockNum == bi & testData$trialType == 'rl' & testData$attentionRunNum != 1)
    idxRel_freq = which(testData$subject == subjNums[ai]& testData$blockNum == bi & testData$trialType == 'freq'& testData$attentionRunNum != 1)
    idxRel_infreq = which(testData$subj/ect == subjNums[ai]& testData$blockNum == bi & testData$trialType == 'infreq'& testData$attentionRunNum != 1)
    idxPrev_freq = which(testData$subject == subjNums[ai]& testData$blockNum == (bi-1) & testData$trialType == 'freq'& testData$attentionRunNum != 1)
    idxPrev_infreq = which(testData$subject == subjNums[ai]& testData$blockNum == (bi-1) & testData$trialType == 'infreq'& testData$attentionRunNum != 1)
    ...
    rlData$meanRT_freq_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = avgRT
    rlData$sdRT_freq_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = sdRT
    rlData$accuracy_infreq_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = attAccuracy
    rlData$threeFreq_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = threeRT
    rlData$accuracy_infreq_prev_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = attAccuracy_prev
    rlData$coefVar_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = CV
    rlData$coefVar_prev_noone[which(rlData$blockNum == bi-1 & rlData$subject == subjNums[ai])] = CV_prev
    
  }
}

rlData$avgRT_detrend_noone = NA
rlData$coefVar_detrend_noone = NA
rlData$accuracy_infreq_detrend_noone = NA
rlData$coefVar_prev_detrend_noone = NA
rlData$accuracy_infreq_prev_detrend_noone = NA
for(si in 1:N_SARL){
  subData = rlData %>% filter(subject == subjNums[si], !is.na(coefVar),!is.na(accuracy_infreq)) # get subject data
  sub_avgRT = c(t(subData$meanRT_freq_noone)) # get rt column and transpose
  sub_coefVar = c(t(subData$coefVar_noone)) # get cv column and transpose
  sub_accuracy_infreq = c(t(subData$accuracy_infreq_noone))# get accuracy column and transpose
  sub_coefVar_prev = c(t(subData$coefVar_prev_noone)) # get cv column and transpose
  ...
  rlData$avgRT_detrend_noone[rlData$subject == subjNums[si]] = sub_avgRT_detrend
  rlData$coefVar_detrend_noone[rlData$subject == subjNums[si]] = sub_coefVar_detrend
  rlData$accuracy_infreq_detrend_noone[rlData$subject == subjNums[si]] = sub_accuracy_infreq_detrend
  rlData$coefVar_prev_detrend_noone[rlData$subject == subjNums[si]] = sub_coefVar_prev_detrend
  rlData$accuracy_infreq_prev_detrend_noone[rlData$subject == subjNums[si]] = sub_accuracy_infreq_prev_detrend  
}
...
```{r, echo=F, message=F, warning=F, error=T, fig.width=5, fig.height=5, fig.align='center'}
# add another way to do bins
attData_noNan_pSARL$trialBins_new = NA
attData_noNan_pSARL$trialBins_new[attData_noNan_pSARL$attentionRunNum_fixed <= 7] = 1
attData_noNan_pSARL$trialBins_new[attData_noNan_pSARL$attentionRunNum_fixed > 7] = 2
attData_noNan_pSARL$trialBins_new[attData_noNan_pSARL$attentionRunNum_fixed >12] = 3

# calculate CV in bins for rewarded and nonrewarded
rewoverBins_new_pSARL = attData_noNan_pSARL %>% group_by(subject,blockNum,rew,trialBins_new) %>% dplyr::summarise(cv = sd(rt_detrend)/mean(rt_detrend), sdRT = sd(rt_detrend)) %>% filter(!is.na(rew),!is.na(cv)) %>% group_by(subject,rew, trialBins_new) %>% dplyr::summarise(avgCV = mean(cv,na.rm = TRUE), sdRT = mean(sdRT))
rewoverBins_new_pSARL_summary = summarySEwithin(data.frame(rewoverBins_new_pSARL),measurevar="avgCV", withinvars=c("rew", "trialBins_new"),idvar="subject", na.rm=FALSE, conf.interval=.95)
ggplot(rewoverBins_new_pSARL_summary, aes(x = trialBins_new, y = avgCV, color = rew))+geom_point()+geom_errorbar(aes(ymin = avgCV-se, ymax = avgCV+se))

rewoverBins_new_sdrt_pSARL_summary = summarySEwithin(data.frame(rewoverBins_new_pSARL),measurevar="sdRT", withinvars=c("rew", "trialBins_new"),idvar="subject", na.rm=FALSE, conf.interval=.95)
ggplot(rewoverBins_new_sdrt_pSARL_summary, aes(x = trialBins_new, y = sdRT, color = rew))+geom_point()+geom_errorbar(aes(ymin = sdRT-se, ymax = sdRT+se))

# calculate CV in bins for rewarded and nonrewarded
rewoverBins_new_noblocknum = attData_noNan_pSARL %>% group_by(subject,rew,trialBins_new) %>% dplyr::summarise(cv = sd(rt_detrend)/mean(rt_detrend)) %>% filter(!is.na(rew),!is.na(cv))
...
# add another way to do bins
attData_noNan_pSARL$trialBins_two = NA
attData_noNan_pSARL$trialBins_two[attData_noNan_pSARL$attentionRunNum <= 10] = 1
attData_noNan_pSARL$trialBins_two[attData_noNan_pSARL$attentionRunNum > 10] = 2

# calculate CV in bins for rewarded and nonrewarded
rewoverBins_two = attData_noNan_pSARL %>% group_by(subject,blockNum,rew,trialBins_two) %>% dplyr::summarise(cv = sd(rt_detrend)/mean(rt_detrend)) %>% filter(!is.na(rew), !is.na(cv)) %>% group_by(subject,rew, trialBins_two) %>% dplyr::summarise(avgCV = mean(cv))
rewoverBins_two_summary = summarySEwithin(rewoverBins_two,measurevar="avgCV", withinvars=c("rew", "trialBins_two"),idvar="subject", na.rm=FALSE, conf.interval=.95)
rewoverBins_two_across = rewoverBins_two %>% group_by(rew,trialBins_two) %>% dplyr::summarise(avgCV = mean(avgCV, na.rm = TRUE))
...
```{r, echo=F, message=F, warning=F, error=T, fig.width=5, fig.height=5, fig.align='center'}
# sanity check on detrended rts
cvComp_pSARL = attData_noNan_pSARL %>% filter(!is.na(rew)) %>% group_by(subject,rew) %>% dplyr::summarise(cv = sd(rt_detrend)/mean(rt_detrend), sdRT = sd(rt_detrend), avgRT = mean(rt_detrend))
ggplot(cvComp_pSARL, aes(x = rew, y = cv))+geom_bar(stat = 'summary', fun.y = 'mean')+geom_point()

cvComp_pSARL_summary = summarySEwithin(cvComp_pSARL,measurevar="cv", withinvars=c("rew"),idvar="subject", na.rm=FALSE, conf.interval=.95)
ggplot(cvComp_pSARL_summary, aes(x = rew, y = cv))+geom_point()+geom_errorbar(aes(ymin= cv-se, ymax = cv+se))

cvComp_pSARL_rt_summary = summarySEwithin(cvComp_pSARL,measurevar="sdRT", withinvars=c("rew"),idvar="subject", na.rm=FALSE, conf.interval=.95)
ggplot(cvComp_pSARL_rt_summary, aes(x = rew, y = sdRT))+geom_point()+geom_errorbar(aes(ymin= sdRT-se, ymax = sdRT+se))
