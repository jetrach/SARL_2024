...
# read in data
flag = 2 #1 - compile all the data from scratch (could take 5 min); 2 - use compiled data in folder (raw subject data concatenated and with additional columns for analysis already added)
...
if(flag == 1){
  ...
  # demoData = read.csv('~/Documents/Github/SARL_2024/analysis/Expt1/SARL_Expt1_Demographics.csv')
  # testData$sex = NA
  # testData$age = NA
  # 
...
for(ai in 1:N_all){
  ...
  RPEs = read.csv('~/Documents/Github/SARL_2024/analysis/Expt1/SARL_Expt1_RPE_1a.csv')
  RPEs_two = read.csv('~/Documents/Github/SARL_2024/analysis/Expt1/SARL_Expt1_RPE_2a.csv')
  modelInfo = read.csv('~/Documents/Github/SARL_2024/analysis/Expt1/SARL_Expt1_modelingInfo.csv')
...
ggplot(trailingCV, aes(x = corrIncorr, y = cv))+geom_bar(stat = "summary", fun = "mean")+geom_point(alpha = 1)+labs(y = 'Trailing average cv(RT) (ms)')+scale_x_discrete(name = 'Infrequent trial accuracy', labels = c('Correct', 'Incorrect'))+theme(axis.title.x = element_text(size = 18, face = 'bold'), axis.title.y = element_text(size = 18, face = 'bold'),axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14), axis.line = element_line(),panel.grid = element_blank(),panel.background = element_rect(fill = "white", colour = "black"),panel.border = element_rect(fill = NA, colour = "white"))+ scale_y_continuous(expand = c(0,0),limits = c(0,.7))
#ggsave('expt1_trailingCV_dots.eps')
...
ggplot(corrInfreq,aes(x = corrInfreq$accuracyInfreq*100, y = corrRL$accuracyRL*100))+geom_point()+geom_smooth(method = 'lm',se = FALSE)+stat_cor(method = "pearson")+labs(y = '% Optimal Choice (RL Trials)',x = '% Correct (Infrequent Attention Trials)')+theme(axis.title.x = element_text(size = 18, face = 'bold'), axis.title.y = element_text(size = 18, face = 'bold'),axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14), axis.line = element_line(),panel.grid = element_blank(),panel.background = element_rect(fill = "white", colour = "black"),panel.border = element_rect(fill = NA, colour = "white"))
...
ggplot(cvFreq,aes(x = cvFreq$cv, y = corrRL$accuracyRL*100))+geom_point()+geom_smooth(method = 'lm',se = FALSE)+stat_cor(method = "pearson")+labs(y = '% Optimal Choice (RL Trials)',x = 'CV (frequent attention trials)')+theme(axis.title.x = element_text(size = 18, face = 'bold'), axis.title.y = element_text(size = 18, face = 'bold'),axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14), axis.line = element_line(),panel.grid = element_blank(),panel.background = element_rect(fill = "white", colour = "black"),panel.border = element_rect(fill = NA, colour = "white"))
shapiro.test(cvFreq$cv) # check if data is normally distributed

cor.test(cvFreq$cv,corrRL$accuracyRL, method = "pearson")
```
...
totCorrels$accuCorrel_fisher = fisher_transform(totCorrels$accuCorrel)
totCorrels$cvCorrel_fisher = fisher_transform(totCorrels$cvCorrel)

t.test(totCorrels$accuCorrel, mu = 0)
t.test(totCorrels$cvCorrel, mu = 0)

t.test(totCorrels$accuCorrel_fisher, mu = 0)
cohens_d(totCorrels,accuCorrel_fisher~1, mu = 0)

t.test(totCorrels$cvCorrel_fisher, mu = 0)
cohens_d(totCorrels,cvCorrel_fisher~1, mu = 0)
...
attentionmetrics$correlSA_fisher = fisher_transform(attentionmetrics$correlSA)
attentionmetrics$correlSA_detrend_fisher = fisher_transform(attentionmetrics$correlSA_detrend)

t.test(attentionmetrics$correlSA,mu=0)
t.test(attentionmetrics$correlSA_detrend,mu=0)

t.test(attentionmetrics$correlSA_fisher,mu=0)
cohens_d(attentionmetrics,correlSA_fisher~1, mu = 0)

t.test(attentionmetrics$correlSA_detrend_fisher,mu=0)
cohens_d(attentionmetrics,correlSA_detrend_fisher~1, mu = 0)
...
avgEffect = freqTrials_terc_rewSign_wide %>% group_by(terc) %>% dplyr::summarise(avg = mean(cv_detrend_diff))
